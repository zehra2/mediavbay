...
<div class="x-form publisher" data-handle="{$_handle}" {if $_id}data-id="{$_id}" {/if}>
  ...
  <div class="publisher-slider">
    ...
    <div class="publisher-footer">
      {if ($_handle == "me" && $user->_data['can_add_anonymous_posts']) || ($_handle != "page" && $user->_data['can_receive_tip']) || (in_array($_handle, ['me', 'page', 'group']) && $_node_can_monetize_content && $_node_monetization_enabled && $_node_monetization_plans > 0) || (in_array($_handle, ['me', 'page', 'group','event']) && $user->_data['can_monetize_content'] && $user->_data['user_monetization_enabled']) }
        <!-- publisher-options -->

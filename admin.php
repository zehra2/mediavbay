  ...
  switch ($_GET['view']) {
    ...
    case 'blogs':
      ...
      switch ($_GET['sub_view']) {
        case '':
          ...
          if ($get_rows->num_rows > 0) {
            while ($row = $get_rows->fetch_assoc()) {
              $row['article_title_url'] = get_url_text($row['title']);
              /* check the post author type */
              if ($row['user_type'] == "user") {
                /* user */
    ...
    case 'movies':
      ...
      switch ($_GET['sub_view']) {
        ...
        case 'payments':
          // page header
          page_header($control_panel['title'] . " &rsaquo; " . __("Movies") . " &rsaquo; " . __("Payments"));

          // calculate earnings
          $total_earnings = 0;
          $month_earnings = 0;
          $get_rows = $db->query("SELECT * FROM movies_payments INNER JOIN users ON movies_payments.user_id = users.user_id INNER JOIN movies ON movies_payments.movie_id = movies.movie_id") or _error('SQL_ERROR');
          if ($get_rows->num_rows > 0) {
            while ($row = $get_rows->fetch_assoc()) {
              $total_earnings += $row['price'];
              if (date('m', strtotime($row['payment_time'])) == date('m')) {
                $month_earnings += $row['price'];
              }
            }
          }
    ...
    case 'market':
      ...
      switch ($_GET['sub_view']) {
        ...
        case 'orders':
          ...
          require('includes/class-pager.php');
          $params['selected_page'] = (!isset($_GET['page']) || (int) $_GET['page'] == 0) ? 1 : $_GET['page'];
          $total = $db->query("SELECT COUNT(*) as count FROM orders INNER JOIN users u1 ON orders.buyer_id = u1.user_id INNER JOIN users u2 ON orders.seller_id = u2.user_id") or _error('SQL_ERROR');
          $params['total_items'] = $total->fetch_assoc()['count'];
          $params['items_per_page'] = $system['max_results'];
        ...
        case 'find_orders':
          ...
          require('includes/class-pager.php');
          $params['selected_page'] = (!isset($_GET['page']) || (int) $_GET['page'] == 0) ? 1 : $_GET['page'];
          $total = $db->query(sprintf('SELECT COUNT(*) as count FROM orders INNER JOIN users u1 ON orders.buyer_id = u1.user_id INNER JOIN users u2 ON orders.seller_id = u2.user_id WHERE orders.order_hash LIKE %1$s', secure($_GET['query'], 'search'))) or _error('SQL_ERROR');
          $params['total_items'] = $total->fetch_assoc()['count'];
          $params['items_per_page'] = $system['max_results'];
          $params['url'] = $system['system_url'] . '/' . $control_panel['url'] . '/market/find_orders?query=' . $_GET['query'] . '&page=%s';
          $pager = new Pager($params);
          $limit_query = $pager->getLimitSql();
          $get_rows = $db->query(sprintf('SELECT orders.*, u1.user_name AS buyer_username, u1.user_firstname AS buyer_firstname, u1.user_lastname AS buyer_lastname, u1.user_picture AS buyer_user_picture, u1.user_gender AS buyer_user_gender, u2.user_name AS seller_username, u2.user_firstname AS seller_firstname, u2.user_lastname AS seller_lastname, u2.user_picture AS seller_user_picture, u2.user_gender AS seller_user_gender FROM orders INNER JOIN users u1 ON orders.buyer_id = u1.user_id INNER JOIN users u2 ON orders.seller_id = u2.user_id WHERE orders.order_hash LIKE %1$s ' . $limit_query, secure($_GET['query'], 'search'))) or _error('SQL_ERROR');
          if ($get_rows->num_rows > 0) {
            while ($row = $get_rows->fetch_assoc()) {
              /* prepare buyer's info */
              $row['buyer_fullname'] = ($system['show_usernames_enabled']) ? $row['buyer_username'] : $row['buyer_firstname'] . ' ' . $row['buyer_lastname'];
        ...
        case 'payments':
          ...
          // get data
          $get_rows = $db->query("SELECT market_payments.*, users.user_id, users.user_name, users.user_firstname, users.user_lastname, users.user_gender, users.user_picture FROM market_payments INNER JOIN users ON market_payments.user_id = users.user_id WHERE market_payments.status = '0'") or _error('SQL_ERROR');
          if ($get_rows->num_rows > 0) {
            while ($row = $get_rows->fetch_assoc()) {
              $row['user_picture'] = get_picture($row['user_picture'], $row['user_gender']);
              switch ($row['method']) {

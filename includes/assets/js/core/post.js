...
$(function () {
  ...
  $('body').on('click', '.js_publisher-review', function () {
    ...
    /* button loading */
    button_status(_this, "loading");
    $.post(api['modules/review'], { 'do': 'publish-review', 'id': publisher.data('id'), 'rating': publisher.find('input[name="rating"]:checked').val(), 'review': publisher.find('textarea').val(), 'photos': JSON.stringify(publisher.data('photos')) }, function (response) {
      if (response.callback) {
        /* button reset */
        button_status(_this, "reset");
        eval(response.callback);

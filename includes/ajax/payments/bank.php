...
try {
  ...
  switch ($_POST['handle']) {
    ...
    case 'movies':
      ...
      // process
      $db->query(sprintf("INSERT INTO bank_transfers (user_id, handle, plan_id, price, bank_receipt, time) VALUES (%s, 'movies', %s, %s, %s, %s)", secure($user->_data['user_id'], 'int'), secure($_POST['id'], 'int'), secure($movie['price']), secure($_POST['bank_receipt']), secure($date))) or _error('SQL_ERROR_THROWEN');
